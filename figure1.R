# Plot expected spread size dp algorithms using Erdos-Renyi network 

df1<-read.csv('./data_erdos_renyi/exp_mech.csv')
df2<-read.csv('./data_erdos_renyi/randomized_version.csv')
df3 <- read.csv('./data_erdos_renyi/greedy_alg.csv')
greedy_vals=read.csv('./data_erdos_renyi/greedy_alg_reference.csv')
...
df$s<- factor(df$s, levels = c('k = 2', 'k = 4'), ordered = TRUE) 
hline_dat = data.frame(s=c('k = 2', 'k = 4'),
...
g<- ggplot(df, aes(x=m, y=ixs_mu, colour=algorithm)) + 
  ...
  geom_point(aes(shape=(epsilon), size=0.3), size=2,na.rm =T,alpha=0.8)+
  geom_line(aes(group=interaction(epsilon, algorithm)),alpha=0.8,size=0.3) +
  geom_hline(data=hline_dat, aes(yintercept=threshold,linetype = "greedy\nbaseline"), 
             colour="#A21212")+
...
saving_name="erdos_renyi_k24-exp-rr.pdf"
