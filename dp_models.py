...
def fill_matrix_c_regularization(list_k,epsilon,penalty_values):
    ...
    keys_c=list(product(list(range(int(max(list_k)))),penalty_values))
    dict_matrices_c_regularized=dict.fromkeys(keys_c)

    for k,penalty in keys_c:

        l=k+1
        C = np.zeros((l, l))
        rho = 1 / (np.exp(epsilon) + 1)
        ...
        gamma=penalty*np.eye(C.shape[0])

        dict_matrices_c_regularized[(k,penalty)]=(np.linalg.inv(C.T@C + gamma.T@gamma))@C.T

    return dict_matrices_c_regularized

def compute_exp_spread_randomized_l2_regularization(iter_matrix, m, k, epsilon,penalty,
                                                    dict_matrices_c_regularized,runs_alg):
    ...
    for t in range(runs_alg):
        ...
        if m == 0:
            ...
            for i in range(k):
                ...
                matrix_C = dict_matrices_c_regularized[(i, penalty)]
                vector_f = matrix_C @ vector_f_tilde
