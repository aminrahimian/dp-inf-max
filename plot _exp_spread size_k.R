...
df<-read.csv('randomized_version.csv')
colnames(df)



df1 <-df %>%mutate(algorithm= rep('randomize',dim(.)[1])) %>%  
  mutate_at(c('m', 'k','epsilon','algorithm'), as.factor) %>% 
  filter(k==4)

df2<- df %>%mutate(algorithm= rep('randomize',dim(.)[1])) %>%  
  mutate_at(c('m', 'k','epsilon','algorithm'), as.factor) %>% 
  filter(k==4)
...
g<- ggplot(df, aes(x=m, y=ixs_mu, colour=algorithm, fill=algorithm)) + 
  geom_errorbar(aes(ymin=ixs_mu-(1.96/(sqrt(ixs_n)))*ixs_sd, ymax=ixs_mu+(1.96/(sqrt(ixs_n)))*ixs_sd), width=.1) +
...
saving_name="comparison_k.pdf"

ggsave('comparison_k.pdf',
       g, width = 5, height = 4)
